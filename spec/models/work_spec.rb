...
describe Work do
  describe "when merging a list of ids" do
    ...
    it "should correctly merge multiple items" do
      ...
      @billed1 = FactoryBot.create(:work, client: @client, start: Time.now + 1.hour, finish: Time.now + 2.hour)
      @billed2 = FactoryBot.create(:work, client: @client)
      ...
      -> { Work.find(@billed1.id) }.should_not raise_error
      -> { Work.find(@billed2.id) }.should raise_error(ActiveRecord::RecordNotFound)
    end
  end

  describe "when converting to an adjustment" do
    it "should work" do
      @work = FactoryBot.create(:work, start: Time.now, finish: Time.now + 2.hours, rate: 50)
      @work.to_adjustment!
  ...
  describe "billed" do
    before(:each) do
      start = Time.now
      finish = start + 1.hour
      @line_item = FactoryBot.create(:work, start: start, finish: finish, rate: 50)
    end
    ...
    it "should be compared to other line items by start time descending" do
      @billed1 = FactoryBot.create(:work, start: Time.now + 1.hour, finish: Time.now + 2.hour)
      @billed2 = FactoryBot.create(:work)
  ...
  describe "unbilled" do
    it "should be checked" do
      @line_item = FactoryBot.create(:work, invoice: nil)
  ...
  describe "incomplete" do
    before do
      @line_item = FactoryBot.create(:work, invoice: nil, start: 1.day.ago)
